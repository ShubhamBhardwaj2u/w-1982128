...
### 3. `update-datasource.ps1` - Datasource Synchronization
**Purpose:** Synchronizes datasource connection strings from YAML config to BIM file (multi-DB support).

**Usage:**
```
wealth-devops-framework/scripts/cube/update-datasource.ps1 `
  -BimPath "wealth-cube/src/model/Wealth.bim" `
  -Environment "DEV"
```

**YAML Format (`wealth-cube/config/datasources/dev.yml`):**
```yaml
environment: DEV
datasources:
  - name: "EDW_WEALTH_DATAMART"
...
**Key Features:**
- Per-datasource name matching and updates
- Auto-locates config by environment
- Partition reference validation
- DryRun preview (`-DryRun`)
- StrictMode fails on mismatches (`-StrictMode`)
